...
from pinax.account.models import EmailConfirmation, SignupCode
from pinax.account.views import INTERNAL_RESET_URL_TOKEN, PasswordResetTokenView
...
class SignupViewTestCase(TestCase):
    ...
    def test_closed(self):
        with self.settings(ACCOUNT_OPEN_SIGNUP=False):
            ...
            response = self.client.post(reverse("account_signup"), data)
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.template_name, "pinax.account/signup_closed.html")
...
class LoginViewTestCase(TestCase):
    ...
    def test_get(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.template_name, ["pinax.account/login.html"])
    ...
    @override_settings(
        AUTHENTICATION_BACKENDS=[
            "pinax.account.auth_backends.UsernameAuthenticationBackend",
        ]
    )
...
class LogoutViewTestCase(TestCase):
    ...
    def test_get_authenticated(self):
        ...
        response = self.client.get(reverse("account_logout"))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.template_name, ["pinax.account/logout.html"])
...
class ConfirmEmailViewTestCase(TestCase):
    ...
    def test_get_good_key(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.template_name, ["pinax.account/email_confirm.html"])
...
class ChangePasswordViewTestCase(TestCase):
    ...
    def test_get_authenticated(self):
        ...
        response = self.client.get(reverse("account_password"))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.template_name, ["pinax.account/password_change.html"])
